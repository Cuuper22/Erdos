 ...
 * Onboarding: User downloads the installer (.exe / .dmg).
 * Configuration:
   * User launches app.
   * Input: OpenAI/Anthropic API Key OR Local Model URL (Ollama).
   * Input: Maximum Cost Limit (e.g., "Stop after $5").
 * Execution:
   * User clicks "Start Mining".
   * The app downloads the latest "Problem Set" manifest from the official repository.
 * The Loop:
   * The user watches a terminal-like log: "Attempting Problem #104... Compiling... Failed. Retrying... Success! Validating..."
 * Submission:
   * On success, the app automatically zips the artifacts and pushes them to a "Solutions" branch or API endpoint.
4. Technical Specifications
A. The Tech Stack
 * Frontend (GUI): Tauri (Rust + React).
   * Why: Extremely small binary size (<10MB), low RAM usage, native look and feel. Easier to bundle than Electron.
 * Backend (Logic): Python 3.11 (bundled via PyInstaller).
   * Why: Richest ecosystem for LLM orchestration (LiteLLM, LangChain) and file I/O.
 * Proof Assistant: Elan (Lean Version Manager).
   * Why: We must manage the Lean version automatically. The user cannot be expected to install Lean manually.
B. The "Smart" Wrapper (Multi-Agent Logic)
 ...
 * Logic:
   ...
   * If not, download the elan-init.sh (or Windows equivalent) and run it silently in a localized app directory (~/.the-prover/bin).
   * Clone the target repository (e.g., google-deepmind/formal-conjectures) into a cached directory.
